...
**Corrections are the future fine-tuning dataset.** `POST /entries/{id}/corrections` merges the fix into the word in `transcript_json` (preserving `line`/`bbox`, setting `verified: true`) *and* appends a row to the `corrections` table logging model_text → corrected_text. `POST /entries/{id}/verify-line` stamps a whole line's words `verified` ("this line is correct" — training positives). Per `OCR_PIPELINE.md` these feed model training — don't drop them when reworking the correction flow.

**The Entry view (`static/entry.html`, served at `/entry?id=…`) is the correction workbench**: photo beside transcript, per-line bbox overlays tinted by confidence, dotted underlines on low/med words, click-to-correct popover (with alternates when the hosted backend provided them), and per-line verify buttons. The local backend emits `line` + normalized `bbox` (fractions of the EXIF-uprighted image) on every word; hosted-backend words lack them and the UI degrades gracefully. `scripts/export_training_data.py` crops every fully-verified line from the original photo into `training_data/` (gitignored) with a `manifest.jsonl` — the TrOCR fine-tune input format. Thumbnails live at `{entry_id}/thumb.jpg` by convention (written by `storage.py`, served via `/files`).
